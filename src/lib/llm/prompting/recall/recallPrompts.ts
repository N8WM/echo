export class RecallPrompts {
  static planExchange(
    ...
    return [
      ...
      `2) Relevance First: Select the minimum set of quotes (typically 1–4, at least 1 minimum, up to 8 maximum) that together answer the question.`,
      ...
      `3) Quote Integrity: If a quote is too long or contains off-topic parts, prefer a different message if available. Otherwise, long quotes`,
      `   are automatically truncated to 50 words in the final message to the user, and ellipsis added.`,
      `4) Message IDs: Only pass messageId values that exist in the provided messages, and do not quote a message more than once.`,
      `5) Context: Context is used only to help the asker when reading the quotes, it should not contain any information other than what part of the question`,
      ...
      `   and keep it short and to one line. A context is ALWAYS in the form of a question.`,
      ``,
      `SELECTION STRATEGY`,
      ...
      ``,
      `TOOL-USE PROTOCOL`,
      `- Typical Grammar Flow: (context -> userQuote+)*`,
      `- Keep going until the assembled components would let a reader resolve the question, or nothing if not enough info`,
      ``,
      `FORMAT`,
      `Your output should simply be a list of function calls, one per line, with their respective arguments filled out.`,
      ``,
  ...
  static exchangeLoopStart() {
    return [
      ...
      `Go ahead and start adding components, starting with the first one in the list.`,
      `Functions:`,
      `- userQuote(messageId: string) — insert a quoted message by message ID`,
      `- context(content: string) — brief context to align the asked question with the following quote(s), so the user doesn't need to look back at their question`,
      ``,
      `Note that nothing you write in your output has any effect on the exchange, only the functions (tools) you call. Make sure you call every planned function in order before finishing.`
    ].join("\n");
  }
}
